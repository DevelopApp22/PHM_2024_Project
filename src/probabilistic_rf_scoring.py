...
def get_regression_score(pdf_type, pdf_args, true_target, min_scale=1e-6):
    ...
    # --- FIX: Griglia dinamica centrata per intercettare qualsiasi valore (es. -107) ---
    ...
    # In scipy > 1.14 usare simpson(y=y, x=x)
    area = simpson(y, x=x) 
...
def plot_pdf_with_true_target(
    ...
    # --- FIX: Griglia dinamica centrata per intercettare qualsiasi valore per calcolo y_max ---
    loc = kwargs.get("loc", 0.0)
    scale = kwargs.get("scale", 1.0)
    ...
    # In scipy > 1.14 usare simpson(y=y_big, x=x_big)
    area = simpson(y_big, x=x_big)
